...
from PyQt5.QtWidgets import (QApplication, QMainWindow, QWidget, QVBoxLayout, QHBoxLayout,
                             ...
                             QFrame,  QDialog, QFormLayout, QSpinBox, QMenu, QAction,
                             QSplitter)
...
from PyQt5.QtWidgets import QGroupBox, QVBoxLayout, QCheckBox, QSpinBox, QLabel, QHBoxLayout
...
只描述观察到的事实，不进行推测或提供解决方案。特别注意识别车辆的特征，以便后续将不同角度的照片关联到同一辆车。 """
        ...
        return self._call_ollama(prompt, images=[base64_img], thinking_log=thinking_log, max_tokens=1200,
                                 temperature=0.1)
...
"""
    ...
    def setup_performance_panel(self, parent_layout):
        perf_group = QGroupBox("性能模拟（测试用）")
        perf_layout = QVBoxLayout()

        self.enable_sim = QCheckBox("启用耗时模拟")
        perf_layout.addWidget(self.enable_sim)

        # AI 分析延迟（毫秒）
        self.ai_delay_spin = QSpinBox()
        self.ai_delay_spin.setRange(0, 30000)
        self.ai_delay_spin.setSuffix(" ms")
        self.ai_delay_spin.setValue(2000)
        perf_layout.addWidget(QLabel("AI 分析延迟:"))
        perf_layout.addWidget(self.ai_delay_spin)

        # 报告生成延迟
        self.save_delay_spin = QSpinBox()
        self.save_delay_spin.setRange(0, 10000)
        self.save_delay_spin.setSuffix(" ms")
        self.save_delay_spin.setValue(500)
        perf_layout.addWidget(QLabel("报告保存延迟:"))
        perf_layout.addWidget(self.save_delay_spin)

        # 人工填充逐字延迟（毫秒/字符）
        self.type_delay_spin = QSpinBox()
        self.type_delay_spin.setRange(0, 200)
        self.type_delay_spin.setSuffix(" ms/字符")
        self.type_delay_spin.setValue(20)
        perf_layout.addWidget(QLabel("模拟人工填充速度:"))
        perf_layout.addWidget(self.type_delay_spin)
