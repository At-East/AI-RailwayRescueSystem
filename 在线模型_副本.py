...
class RailwayRescueAnalyzer:
    def __init__(self, model_name="qwen3.5:9b", base_url="http://localhost:11434"):
        ...
        self.headers = {"Content-Type": "application/json"}  # Ollama 不需要 Authorization

    def _call_ollama(self, prompt, images=None, thinking_log=None, max_tokens=2000):
        """调用 Ollama API 的统一方法"""
        payload = {
    ...
    def check_railway_related(self, image_path, thinking_log=None):
        ...
        payload = {
            "model": self.model,
            "messages": [
                {"role": "system", "content": """你是一个铁路相关素材识别专家。
                请判断图片中是否包含与铁路相关的内容，如火车、铁轨、铁路设施等。
                只需要返回"是"或"否"，不需要其他解释。"""},
                {"role": "user", "content": [
                    {"type": "text", "text": "这张图片是否包含与铁路相关的内容？只返回是或否。"},
        ...
        try:
            if thinking_log:
                thinking_log.emit("向AI模型发送请求，判断是否与铁路相关")
            ...
            if thinking_log:
                thinking_log.emit(f"AI判断结果: {result['choices'][0]['message']['content']}")

            return "是" in result["choices"][0]["message"]["content"] if "choices" in result else False
        except Exception as e:
            if thinking_log:
                thinking_log.emit(f"判断是否与铁路相关时出错: {str(e)}")
            return False
    ...
    def describe_image_content(self, image_path, thinking_log=None):
        ...
        # 将 system prompt 和 user prompt 合并成一个 prompt
        prompt = """你是一个铁路相关素材识别专家。请判断图片中是否包含与铁路相关的内容，如火车、铁轨、铁路设施等。

        这张图片是否包含与铁路相关的内容？只返回"是"或"否"，不需要其他解释。"""

        # 调用 Ollama API
        result = self._call_ollama(
            prompt=prompt,  # 合并后的提示词
            images=[base64_img],  # 图片列表
        ...
        try:
            ...
            return result["choices"][0]["message"]["content"] if "choices" in result else "无法描述图片内容"
        except Exception as e:
            return f"描述图片内容时出错: {str(e)}"
    ...
    def analyze_scene(self, image_path, thinking_log=None):
        ...
        payload = {
            "model": self.model,
            "messages": [
                {"role": "system", "content": """你是一名资深的铁路救援现场评估专家，仅根据图片事实描述现场情况，要联系上下文不能把同1辆车的不同角度，
                理解为不同的车辆进行分析。不包含任何救援方案或建议。
                描述应包括：
                1. 事故类型（如列车脱轨、碰撞等）
                2. 受影响的车辆数量和位置
                3. 车辆损坏程度和具体受损部位
    ...
    def generate_rescue_steps(self, analysis_result, thinking_log=None):
        # 预设的标准救援步骤框架，包含国吊和液压顶复两部分
        rescue_steps_framework = """救援方案应包含国吊和液压顶复两部分操作步骤：
        ...
        payload = {
            ...
            "messages": [
                ...
                {"role": "user",
                 "content": f"基于以下救援分析，严格按照上述标准步骤框架，生成详细的救援步骤，必须包含国吊和液压顶复两部分：\n{analysis_result}"}
            ],
            "max_tokens": 2000,
            "temperature": 0.4
        }
