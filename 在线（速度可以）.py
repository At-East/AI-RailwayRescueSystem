...
class RailwayRescueAnalyzer:
    def __init__(self, api_key):
        self.api_key = "sk-zoqvknzmshcsoxxntrjekabppzywgzdjwtcgbxfhwnrokpiw"
        self.api_url = "https://api.siliconflow.cn/v1/chat/completions"
        self.headers = {
            "Content-Type": "application/json",
            "Authorization": f"Bearer {self.api_key}"
        }
        self.model = "Qwen/Qwen3-VL-8B-Thinking"
    ...
    def describe_image_content(self, image_path, thinking_log=None):
        ...
        payload = {
            "model": self.model,
            "messages": [
                {"role": "system", "content": """请客观描述图片中的内容，不添加任何主观评价。
                描述应简洁明了，准确反映图片中的主要元素。"""},
                {"role": "user", "content": [
...
class RescueSystemGUI(QMainWindow):
    ...
    def refresh_step(self, step_num):
        ...
        if step_num == 1:
            ...
            self.log_thinking("1. 打开支腿")
            self.log_thinking("2. 升起国吊大臂")
            self.log_thinking("3. 将救援车钩移动至脱轨车辆上方")
            self.log_thinking("4. 将被救援车辆与救援车钩捆绑")
            self.log_thinking("5. 将脱轨车辆恢复至正常轨道上")
            self.log_thinking("6. 收起国吊大臂")
            self.log_thinking("7. 收起国吊支撑臂")
            self.log_thinking("8. 收起支腿")
            self.log_thinking("9. 驶离救援作业区")
