...
class RailwayRescueAnalyzer:
    ...
    def _call_ollama(self, prompt, images=None, thinking_log=None, max_tokens=2000, temperature=0.3):
        """统一的 Ollama API 调用方法"""
        payload = {
        ...
        try:
            if thinking_log:
                thinking_log.emit(f"正在调用 Ollama 模型: {self.model_name}")
            response = requests.post(self.api_url, json=payload, timeout=120)
            response.raise_for_status()
            result = response.json()
            if thinking_log:
                thinking_log.emit("Ollama 模型响应成功")
            return result.get("response", "").strip()
        except requests.exceptions.ConnectionError:
            error_msg = "无法连接到 Ollama 服务，请确保 Ollama 正在运行（ollama serve）"
            if thinking_log:
                thinking_log.emit(error_msg)
            return error_msg
        except Exception as e:
    ...
    def check_railway_related(self, image_path, thinking_log=None):
        """检查图片是否与铁路相关"""
        if thinking_log:
            thinking_log.emit(f"检查图片是否与铁路相关: {os.path.basename(image_path)}")
        base64_img = self.encode_image(image_path)
        if not base64_img:
            return False
        prompt = "这张图片是否包含与铁路相关的内容（火车、铁轨、铁路设施等）？只回答'是'或'否'。"
        result = self._call_ollama(prompt, images=[base64_img], thinking_log=thinking_log, max_tokens=10, temperature=0.1)
        return "是" in result
    ...
    def describe_image_content(self, image_path, thinking_log=None):
        ...
        return self._call_ollama(prompt, images=[base64_img], thinking_log=thinking_log, max_tokens=200)
...
只描述观察到的事实，不进行推测或提供解决方案。特别注意识别车辆的特征，以便后续将不同角度的照片关联到同一辆车。"""
        return self._call_ollama(prompt, images=[base64_img], thinking_log=thinking_log, max_tokens=500, temperature=0.3)
    ...
    def synthesize_scene(self, individual_scenes, thinking_log=None):
        ...
        prompt = f"""整合以下多张图片/视频帧的现场描述，生成统一、连贯的现场情况报告。
...
请生成最终的现场情况报告："""
        return self._call_ollama(prompt, thinking_log=thinking_log, max_tokens=800, temperature=0.3)

    def analyze_rescue_feasibility(self, scene_description, thinking_log=None):
        """分析两种救援方案的可行性"""
        if thinking_log:
            thinking_log.emit("开始进行救援可行性分析")
        prompt = f"""基于以下铁路事故现场情况，分析国吊和液压起伏设备两种救援方案的可行性。
...
注意：不包含具体操作步骤。"""
        return self._call_ollama(prompt, thinking_log=thinking_log, max_tokens=800, temperature=0.4)

    def generate_rescue_steps(self, analysis_result, thinking_log=None):
        """生成详细的救援步骤（严格按照国吊+液压顶复框架）"""
        if thinking_log:
...
4. 应急处理方案
5. 备用方案准备"""
        return self._call_ollama(prompt, thinking_log=thinking_log, max_tokens=1000, temperature=0.4)
...
class RescueSystemGUI(QMainWindow):
    ...
    def init_ui(self):
        ...
        self.step_nav = QLabel("<h3>当前步骤：</h3><p style='color:gray'>1. 导入素材并生成现场描述</p><p style='color:gray'>2. 救援可行性分析</p><p style='color:gray'>3. 制定救援步骤</p><p style='color:gray'>4. 风险点预判</p><p style='color:gray'>5. 生成最终报告</p>")
