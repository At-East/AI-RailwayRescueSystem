...
class RailwayRescueAnalyzer:
    ...
    def _call_ollama(self, prompt, images=None, thinking_log=None, max_tokens=320, temperature=0.1):
        """统一的 Ollama API 调用方法"""
        # 防止Ai喷英文
        ...
        try:
            ...
            if thinking_log:
                thinking_log.emit(f"X {error_msg}")
            return ""
    ...
    def check_railway_related(self, image_path, thinking_log=None):
        ...
        if not base64_img:
            if thinking_log:
                thinking_log.emit("X 图片编码失败")
            return False

        if thinking_log:
            thinking_log.emit(f"Yes 图片编码成功，长度: {len(base64_img)}")
...
只描述观察到的事实，不进行推测或提供解决方案。特别注意识别车辆的特征，以便后续将不同角度的照片关联到同一辆车。 """
        ...
        return self._call_ollama(prompt, images=[base64_img], thinking_log=thinking_log, max_tokens=320,
                                 temperature=0.1)
...
请使用标准的简体中文生成最终的现场情况报告："""
        return self._call_ollama(prompt, thinking_log=thinking_log, max_tokens=320, temperature=0.1)
...
注意：不包含具体操作步骤，且国吊方案不作为主方案。"""
        return self._call_ollama(prompt, thinking_log=thinking_log, max_tokens=320, temperature=0.2)
    ...
    **推荐方案**：液压起伏设备救援方案
    ...
    """
        ...
        try:
            ...
            def on_open():
                subprocess.run(['open', '/Users/dadada/Documents/铁路事故救援方案_202604.docx'])
                dlg.accept()
...
if __name__ == "__main__":
    ...
    app.setStyle('Fusion')  #使用Fusion可以解决之前从Windows迁移时候的UI布局问题（有一点问题不过暂时可以接受）
...
#这是几行注释，看起来让自己有事情做
    ...
    #关注塔菲喵，关注塔菲谢谢喵
        ...
        #组了个乐队，咕咕又嘎嘎
            ...
            #尘缘难尽默对沉香，龙珠何时在胸膛
            #秉承千年卓绝意志，潜修东南山
            #香火在雨中烧几十个暑和寒，血脉相传一方苦行山
            #龙珠九转十二金光，返指五岳和三江
            #不怕困难信赖XX，We are 挺XX
            #挺XX，要打拼，XXXXXXXXX
            #庆历四年春，滕子京谪守巴陵郡，越明年，政通人和，百废俱兴，乃重修岳阳楼，增其旧制，ketangsongjinren